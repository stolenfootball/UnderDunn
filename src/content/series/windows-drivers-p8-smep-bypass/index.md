...
date: 2025-08-19T21:45:44-04:00
description: "Porting the HEVD buffer overflow exploit to 64-bit Windows 8.1 and using ROP to bypass Supervisor Mode Execution Prevention."
draft: false
tags:
  - pwn
  - rev
...
I always start by writing a quick program to exercise the vulnerable IOCTL without triggering the overflow.  This allows us to see what the program looks like while its working, as well as double check any offsets we'll need to write the exploit.  An example of such a program is in the last post, so I'm not going to reproduce it here.
...
Let's set a breakpoint just before the `memmove` in `TriggerBufferOverflowStack`, then run the program.
...
Just before the `memmove` in IDA, we can see that a pointer to `KernelBuffer` is loaded into `rcx`.  (IDA "helpfully" lables this `UserBuffer` because `UserBuffer` was passed in through `rcx` as an argument, but at this point in the program, `UserBuffer` is actually in `rdi`, and IDA is just messing with us).  
...
The three callee saved registers are set to all `A`, which is exactly what we expected.  In addition, the top value on the stack (which is the return address) is set to all `B` at the time of the crash.  Looks like our reverse engineering from above was correct!
...
Here is the token stealing shellcode for Windows 8:
...
Then open `shellcode.o` with [HxD](https://mh-nexus.de/en/hxd/), highlight the bytes in the text window, and do `Edit` -> `Copy as` -> `C`, then simply paste them into the exploit code.

![Copy bytestring](./images/7_copy_bytestring.gif)

The byte string with the shellcode won't be executable by default, so we'll need to allocate some executable memory for it and copy it over.  This can be done with the `VirtualAlloc` and `RtlCopyMemory` functions:
...
Take note of the last argument to `VirtualAlloc`, which assigns `RWX` permissions to the memory area we're allocating for the shellcode.

Here is the reworked x64 version of the exploit code so far:
...
The easiest way to flip the 20th bit of the CR4 register in this situation is with a ROP chain.
...
If use Python to get the same result, we'll see the new value `CR4` needs to be set to is `0x406f8`.
...
Ultimately, with the macros in place, all we have to do is define `CR4_ORIG_VALUE` to be the same as the `CR4` value we found with WinDbg.

## ROP to disable SMEP
Since gadget offsets change with just about every version of the Windows kernel, when making a kernel ROP chain, a good first step is always to document which version we're working with in case anyone wants to port our exploit to a different version of Windows down the line.
...
We can see we got lucky.  The only thing the calling function does after we return is clean up its own stack and return.  This means we have `0x28` bytes after the return address to work with for our ROP chain.
...
## Final thoughts
That was a long post (and significantly more technical than the previous ones), but hopefully you were able to learn something from it.  The next post will be about bypassing VBS, the newest mitigation for local kernel exploitation.  Once you can do that, you can start writing exploits for brand new Windows 11 systems.
