...
date: 2025-08-15T14:37:47-04:00
description: "Developing a kernel stack buffer overflow exploit against HackSysExtremeVulnerableDriver on 32-bit Windows 7 with token-stealing shellcode."
draft: false
tags:
  - pwn
  - rev
...
This function is more or less just a parser for the IRP that passes the input buffer and its length into the function I called "TriggerOverflow".  Let's look at that next:
...
Before you compile the program, you'll need to run the following in the Developer command prompt to set it to compile code to 32 bit binaries.
...
Set the breakpoint in WinDbg and run the client code on the VM.
...
We can tell by looking at the assembly that a pointer to the vulnerable buffer is in `eax`, and if we're overwriting the return address, we need to overwrite `ebp+4` through `ebp+8`.  Let's subtract `eax` from `ebp` to see how far we have to go.
...
We also need to change the `TriggerExploit` function to look like this:
...
For those coming from Linux user mode exploit development, the standard next step would be to set your registers and make a syscall.  Windows shellcode is a bit more complex since it doesn't make direct use of the syscall interface as much and instead you have to do some dynamic loading from libraries, but it would still be possible.  
...
The `FS` register is a special register that always points to the **TIB** ([Thread Information Block](https://en.wikipedia.org/wiki/Win32_Thread_Information_Block)) of the currently in-context thread in 32 bit Windows.  The `TIB` is a large struct that stores information such as the stack and heap locations, last thrown error, and more.  Each thread has it's own `TIB`. 

The `TIB` also contains a pointer to the [`KTHREAD`](https://www.geoffchappell.com/studies/windows/km/ntoskrnl/inc/ntos/ke/kthread/index.htm) struct, which contains information about the kernel-space portion of the thread.  In this case however, we only care the `KHTHREAD` struct also contains a pointer to our process's `EPROCESS` struct.
...
Next, we need to find the kernel's `EPROCESS` struct.  This is fairly easy, because Windows stores all `EPROCESS` structs in a circular doubly linked list, and the kernel's PID is always 4.  This means we can just loop through every process on the computer, starting from our exploit code's process, until we find the kernel's process.
...
When writing exploit code it is even more difficult than normal to document what your code is doing, since the program being exploited isn't included in the source.  As such, it is extremely important to include proper comments and explanations for everything non-obvious that the code is doing.
