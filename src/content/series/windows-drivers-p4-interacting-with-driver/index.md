...
date: 2025-07-20T11:46:16-04:00
description: "How IRPs, IO Stack Locations, and dispatch functions work together to let user-mode applications communicate with kernel drivers."
draft: false
tags:
  - pwn
  - rev
...
Windows represents all system resources through a data structure called an **object**.  An object can be a file, thread, graphical image, or even a physical device.  All the object does is provide a consistent interface to access the resource.

User space code can't directly interface with objects.  Instead, it must obtain a **handle** to the object, which is simply a pointer that can be used to examine or modify the object.

The function used to open a handle to an object is the somewhat incongruously named `CreateFile` function.  `CreateFile` doesn't create a file by default.  Instead, it looks for an object with the name given and returns a handle to it if possible.  It's only if the object doesn't exist that it will fall back to making a new file.
...
Because of this, if the symlink creation fails, we call `IoDeleteDevice` and clean up the device object we created earlier before exiting.  We'll also need to update the **Unload** function, which now has some cleanup work to do.
...
> 
> This is due to a small idiosyncracy of the Windows driver model - we need to be able to open and close a handle to our driver.  This is done by sending Create and Close IRPs to the driver.  If `IRP_MJ_CREATE` and `IRP_MJ_CLOSE` aren't handled, we won't be able to obtain a handle even if we made the Device Object, and the driver will be inaccessible.
>
