...
date: 2025-07-22T13:20:31-04:00
description: "Building a driver that handles DeviceIoControl requests and exploring buffered, direct, and neither I/O methods."
draft: false
tags:
  - pwn
  - rev
...
This will be the last post in the "driver development crash course" part of this series.  The goal of these posts isn't to make a great driver developer, but rather to give enough of a baseline understanding of how driver code works that it will be possible to follow the later rev and pwn posts without becoming too lost.

If you're interested in more driver development lessons beyond this, as always, [Pavel Yosifovich's Windows Kernel Programming](https://www.amazon.com/Windows-Kernel-Programming-Pavel-Yosifovich/dp/B0BW2X91L2) is your best bet.
...
## IOCTLs
Nearly any time you create a driver, you'll want it to be able to perform multiple functions.  For example, in an antivirus driver, there are any number of things you may want to do, but you don't want to make a new driver for each one.  Some of these could be:

- Protect a file from deletion
- Write to an alternate data stream
- Hide a file from the operating system

This is accomplished with the **IOCTL**, or **IO Control Code**, which is a number passed in with the IRP that tells the driver which action to take.  They are typically handled by a large switch statement in the DeviceControl dispatch function, which has different actions to take depending on which IOCTL is received.

Each operation the driver can perform gets a code, and you pass the code in through the `DeviceIoControl` function when calling the driver.
...
- `DeviceType` - a [large enum](https://learn.microsoft.com/en-us/windows-hardware/drivers/kernel/specifying-device-types) that specifies the type of device the driver is.  For a software driver such as ours, the value can be set to either anything over `0x8000` or `0x22`, which indicates a software device.
...
Virtual memory is implemented through pages and [page tables](https://en.wikipedia.org/wiki/Page_table).  The physical memory of the computer is separated into 4096 byte blocks called pages, which is the smallest unit of memory a process can request.
...
Pages can also be swapped out to disk if the computer is asking for more physical memory than is currently available.  Any access to a page that is not currently residing in RAM results in a **page fault**, during which the operating system will pause the current program and retrieve the page before continuing.
...
The process space virtual memory that the kernel sees depends on the currently in-context process, as can be seen in the graphic above.  The OS accomplishes this by keeping a pointer to the current page table structure in the `CR3` register, and swapping it out with a pointer to the new process's page table structure on a context switch.
...
There are even more problems that could occur when accessing user memory from kernel space, but I'm not going to go into them here.  This should at least provide enough motivation to convince you that it isn't trivially safe.
