...
date: 2025-07-17T19:45:29-04:00
description: "Setting up a driver development environment in Visual Studio and writing a minimal kernel driver that loads and unloads."
draft: false
tags:
  - pwn
  - rev
...
## Loading the Driver
Now that we have a driver, we have to load it. (You can think of loading the driver as "installing" it in the operating system).
...
We can bypass the requirement by enabling test signing.  **Make sure to do this on the VM, not your local machine**.  To enable test signing, run the following then reboot the system:
...
Open the Release directory in an Administrative command prompt, then run the following command to install the driver.  

> Use `CMD` not Powershell to do this.  Powershell will not interpret the command correctly.

```ps1
sc create sample type= kernel start= auto binPath= C:\Users\[YOUR USER]\repos\[YOUR DRIVER NAME]\x64\debug\[YOUR DRIVER NAME].sys
```

Note the locations of the spaces and the equals signs after `type`, `start`, and `binPath`.  Yes it is a bit weird, but it won't work if the spaces aren't attached correctly.
